...
oracle_j2sdk_detect() {
  ...
  # GA release of 9+
  if [[ $archive_name =~ jdk-([0-9]+)(u([0-9]+))?_linux-(i586|x64|amd64|arm-vfp-hflt)_bin\.tar\.gz ]]
  then
    j2se_release=${BASH_REMATCH[1]}
    j2se_update=${BASH_REMATCH[3]}
    j2se_arch=${BASH_REMATCH[4]}
...
oracle_j2sdk_control() {
    build_depends="libasound2, libgl1-mesa-glx, libgtk-3-0, libcairo-gobject2, libxslt1.1, libxtst6, libxxf86vm1"
    j2se_control
